...
Route::get("/tools/users/{user}/profile", [UserController::class, "user_profile2"])->middleware("auth");

// Show User Location Form
Route::get("/users/{user}/user-location", [UserController::class, "user_location"])->middleware("auth");
Route::get("/tools/users/{user}/location", [UserController::class, "user_location2"])->middleware("auth");
...
Route::get("/tools/users/map", [UserController::class, "map"])->middleware("auth");
...
Route::get("/tools/spots/create", [SpotController::class, "create2"])->middleware("auth");

// Show Spot Profile
Route::get("/spots/get-profile", [SpotController::class, "getProfileFromUser"])->name("spots.get-profile")->middleware("auth");
...
Route::get("/tools/spots/map", [SpotController::class, "map"])->middleware("auth");
...
Route::get('/test-mail', function () {
    Mail::raw('To je testno sporočilo iz Laravel aplikacije prek Gmail SMTP.', function ($message) {
        $message->to('zigakovac42@gmail.com')
            ->subject('Testno sporočilo');
    });

    return 'Sporočilo uspešno poslano!';
});

Route::get("/tools", [ToolController::class, "showIndex"])->middleware("auth");
Route::get("/tools/contacts/create", [ListingController::class, "create2"])->middleware("auth");
Route::get("/tools/contacts/create/test", [ToolController::class, "showCreateTest"])->middleware("auth");
Route::get("/tools/contacts/map", [ListingController::class, "map"])->middleware("auth");
