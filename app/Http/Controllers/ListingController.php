...
//                "description" => "required"
    ...
    public function map()
    {
        ...
        foreach ($listings as $contact) {
            $myPhotoPath = User::where('name', $contact->My_Callsign)->value('profile_photo_path');
            $theirPhotoPath = User::where('name', $contact->Their_Callsign)->value('profile_photo_path');
            // Preveri, ali je myPhotoPath ničen in dodeli vrednost
            $contact->myProfilePhoto = $myPhotoPath ? Storage::url($myPhotoPath) : null;
            $contact->theirProfilePhoto = $theirPhotoPath ? Storage::url($theirPhotoPath) : null;
        }
        return view('listings.map2', compact('listings', 'total'));
    }
